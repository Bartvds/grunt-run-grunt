...
var gruntKnownOptions = require('grunt-known-options');
...
function runGruntfile(grunt, src, tasks, options, callback) {
	...
	var knownOpts = _.flatMap(gruntKnownOptions, function (value, opt) {
		return value.short ? [opt, value.short] : [opt];
	});
	...
	var stdout = typeof options.stdout === 'function' ? options.stdout : null;
	var stderr = typeof options.stderr === 'function' ? options.stderr : null;

	// mix output (this wise?)
	child.stdout.on('data', function (data) {
		if (stdout) {
			stdout.call(this, data);
		}
		mixedStdIO.push(data);
	});
	child.stderr.on('data', function (data) {
		if (stderr) {
			stderr.call(this, data);
		}
		mixedStdIO.push(data);
	});
}
